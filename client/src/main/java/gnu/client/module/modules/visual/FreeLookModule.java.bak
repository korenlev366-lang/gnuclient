...
/**
 * Free Look — Path B: no rotation writes, ASM redirects orientCamera reads.
 *
 * <p>Unlike the CameraHook approach (which wrote camera angles into player rotation
 * before orientCamera and restored after), Path B uses {@link WorldRenderTransformer}
 * to replace every GETFIELD of {@code Entity.rotationYaw/rotationPitch} inside
 * {@code EntityRenderer.orientCamera} with INVOKESTATIC to
 ...
 * Freelook camera angles are accumulated independently from the setAngles
 * arguments received by {@code dispatchSetAngles} (which intercepts
 * {@code EntityPlayerSP.setAngles} inside updateCameraAndRender).
 *
 * <p>{@link gnu.client.runtime.FreeLookHook#dispatchSetAngles} receives the raw
 * setAngles arguments (which are per-frame deltas in 1.8.9, not absolute angles)
 ...
 * baked into the pitch argument reaching dispatchSetAngles, so dPitch already
...
public final class FreeLookModule extends Module implements PacketListener {
    ...
    /**
     * Independent camera angles (accumulated from setAngles deltas received
     * by FreeLookHook.dispatchSetAngles on the render thread).
    ...
    // module. Mouse delta accumulation is handled by dispatchSetAngles on the
    ...
    // ── Camera delta accumulation (called from FreeLookHook.dispatchSetAngles) ─

    /**
     * Apply a single frame's mouse delta (sensitivity-applied, with vanilla
     * invertMouse already baked into pitch) to the independent camera angles.
     *
     * <p>Called by {@link gnu.client.runtime.FreeLookHook#dispatchSetAngles}
     * on the render thread inside updateCameraAndRender, receiving the raw
     * setAngles arguments. In 1.8.9, setAngles takes DELTAS (not absolute
     * angles) and internally does rotationYaw += yaw * 0.15, rotationPitch
     * -= pitch * 0.15, then clamps pitch to [-90, 90]. This method replicates
     * that exact math on the independent cameraYaw/cameraPitch.
     *
     * <p>This method does NOT re-apply invertPitch — vanilla's own invertMouse
     * is already baked into the pitch argument at the call site.
     *
     * @param dYaw   the per-frame yaw delta from setAngles (sensitivity-applied)
    ...
    public void applyCameraDelta(float dYaw, float dPitch) {
        ...
        // Vanilla clamps pitch to [-90, 90] unconditionally after setAngles
