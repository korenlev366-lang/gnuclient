...
void log_jni_exception(JNIEnv* env, const char* where) {
    ...
    env->ExceptionClear();
    log_line(std::string("RND_TRANSFORM_ JNI exception at ") + where);
}
...
bool ensure_transformer_loaded(JNIEnv* env) {
    ...
    jclass local_transformer =
        JniBridge::instance().load_class(env, "gnu.client.runtime.WorldRenderTransformer");
    if (!local_transformer || env->ExceptionCheck()) {
        log_jni_exception(env, "loadClass(WorldRenderTransformer)");
        return false;
    }
    ...
    if (!g_transformer_class) {
        log_line("RND_TRANSFORM_ ensure failed: NewGlobalRef transformer");
        return false;
    }
...
void try_patch_entity_renderer(jvmtiEnv* jvmti, JNIEnv* env, const char* name,
    ...
    log_line(std::string("RND_TRANSFORM_ hook target ") + name
             + " len=" + std::to_string(class_data_len));

    if (!ensure_transformer_loaded(env)) {
        log_line("RND_TRANSFORM_ transformer method unavailable; pass-through");
        return;
    }
    ...
    env->DeleteLocalRef(input);
    if (env->ExceptionCheck()) {
        log_jni_exception(env, "WorldRenderTransformer.transform");
        return;
    }
    if (!output) {
        log_line(std::string("RND_TRANSFORM_ Java transformer pass-through for ") + name);
        return;
    }
    ...
    if (err != JVMTI_ERROR_NONE || !out_buf) {
        log_line("RND_TRANSFORM_ jvmti Allocate failed err=" + std::to_string(err));
        env->DeleteLocalRef(output);
        return;
    }
    ...
    *new_class_data = out_buf;
    log_line("RND_TRANSFORM_ patched EntityRenderer bytes=" + std::to_string(class_data_len)
             + " new_len=" + std::to_string(out_len));
}

bool WorldRenderTransformerInstall::install(JNIEnv* env) {
    if (g_installed.load(std::memory_order_acquire))
        return true;
    if (!env) {
        log_line("RND_TRANSFORM_ install failed: null JNIEnv");
        return false;
    }
    ...
    if (!jvmti || !ctx.has_retransform()) {
        log_line("RND_TRANSFORM_ install failed: JVMTI retransform unavailable");
        return false;
    }

    if (!ensure_transformer_loaded(env)) {
        log_line("RND_TRANSFORM_ install failed: load WorldRenderTransformer");
        return false;
    }
    ...
    if (err != JVMTI_ERROR_NONE || !classes) {
        log_line("RND_TRANSFORM_ GetLoadedClasses failed err=" + std::to_string(err));
        return false;
    }
    ...
    for (jint i = 0; i < class_count; ++i) {
        ...
        if (jvmti->GetClassSignature(classes[i], &sig, nullptr) == JVMTI_ERROR_NONE && sig) {
            if (is_target_signature(sig)) {
                ...
                log_line(std::string("RND_TRANSFORM_ found loaded target ") + sig);
            }
    ...
    if (!targets.empty()) {
        ...
        if (err != JVMTI_ERROR_NONE) {
            ...
            log_line("RND_TRANSFORM_ RetransformClasses failed err=" + std::to_string(err));
            return false;
        }
        log_line("RND_TRANSFORM_ RetransformClasses OK count="
                 + std::to_string(targets.size()) + " hook_hits="
                 + std::to_string(g_transform_hits.load(std::memory_order_relaxed)));
    } else {
        log_line("RND_TRANSFORM_ no loaded EntityRenderer yet; hook will apply on first load");
    }
