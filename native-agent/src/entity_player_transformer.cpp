...
void log_jni_exception(JNIEnv* env, const char* where) {
    ...
    env->ExceptionClear();
    log_line(std::string("ESP_TRANSFORM_ JNI exception at ") + where);
}
...
bool ensure_transformer_loaded(JNIEnv* env) {
    ...
    jclass local_transformer =
        JniBridge::instance().load_class(env, "gnu.client.runtime.EntityPlayerSPTransformer");
    if (!local_transformer || env->ExceptionCheck()) {
        log_jni_exception(env, "loadClass(EntityPlayerSPTransformer)");
        return false;
    }
    ...
    if (!g_transformer_class) {
        log_line("ESP_TRANSFORM_ ensure failed: NewGlobalRef transformer");
        return false;
    }
...
void try_patch_entity_player_sp(jvmtiEnv* jvmti, JNIEnv* env, const char* name,
    ...
    log_line(std::string("ESP_TRANSFORM_ hook target ") + name
             + " len=" + std::to_string(class_data_len));

    if (!ensure_transformer_loaded(env)) {
        log_line("ESP_TRANSFORM_ transformer method unavailable; pass-through");
        return;
    }
    ...
    env->DeleteLocalRef(input);
    if (env->ExceptionCheck()) {
        log_jni_exception(env, "EntityPlayerSPTransformer.transform");
        return;
    }
    if (!output) {
        log_line(std::string("ESP_TRANSFORM_ Java transformer pass-through for ") + name);
        return;
    }
    ...
    if (err != JVMTI_ERROR_NONE || !out_buf) {
        log_line("ESP_TRANSFORM_ jvmti Allocate failed err=" + std::to_string(err));
        env->DeleteLocalRef(output);
        return;
    }
    ...
    *new_class_data = out_buf;
    log_line("ESP_TRANSFORM_ patched EntityPlayerSP bytes=" + std::to_string(class_data_len)
