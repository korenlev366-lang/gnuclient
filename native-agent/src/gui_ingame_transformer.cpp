...
void log_jni_exception(JNIEnv* env, const char* where) {
    ...
    env->ExceptionClear();
    log_line(std::string("GUI_TRANSFORM_ JNI exception at ") + where);
}
...
bool ensure_transformer_loaded(JNIEnv* env) {
    ...
    jclass local_transformer =
        JniBridge::instance().load_class(env, "gnu.client.runtime.GuiIngameTransformer");
    if (!local_transformer || env->ExceptionCheck()) {
        log_jni_exception(env, "loadClass(GuiIngameTransformer)");
        return false;
    }
    ...
    if (!g_transformer_class) {
        log_line("GUI_TRANSFORM_ ensure failed: NewGlobalRef transformer");
        return false;
    }
...
void try_patch_gui_ingame(jvmtiEnv* jvmti, JNIEnv* env, const char* name,
    ...
    log_line(std::string("GUI_TRANSFORM_ hook target ") + name
             + " len=" + std::to_string(class_data_len));

    if (!ensure_transformer_loaded(env)) {
        log_line("GUI_TRANSFORM_ transformer method unavailable; pass-through");
        return;
    }
    ...
    env->DeleteLocalRef(input);
    if (env->ExceptionCheck()) {
        log_jni_exception(env, "GuiIngameTransformer.transform");
        return;
    }
    if (!output) {
        log_line(std::string("GUI_TRANSFORM_ Java transformer pass-through for ") + name);
        return;
    }
    ...
    if (err != JVMTI_ERROR_NONE || !out_buf) {
        log_line("GUI_TRANSFORM_ jvmti Allocate failed err=" + std::to_string(err));
        env->DeleteLocalRef(output);
        return;
    }
    ...
    *new_class_data = out_buf;
    log_line("GUI_TRANSFORM_ patched GuiIngame bytes=" + std::to_string(class_data_len)
